...
def main():
    ...
    with st.expander("더 잘 사용할 수 있는 간단한 팁"):
        st.markdown("- 프롬프트를 작성할 때, 주저하지 말고 길고 자세히 적어보세요. 문장의 길이가 길어질수록 더 많은 정보를 얻을 수 있습니다. \
            - 마음에 드는 Emoji가 있으면, 다운로드 버튼을 눌러서 저장해보세요. \
            - 배경 제거 기능은 아직 beta 버전이기 때문에, 정확도가 떨어질 수 있습니다. \
            - Cfg scale factor는 모델이 얼마나 텍스트의 내용을 반영하는지를 결정하는 파라미터입니다. 25까지 설정 가능하지만, 15 이상의 값은 추천하지 않습니다.")
